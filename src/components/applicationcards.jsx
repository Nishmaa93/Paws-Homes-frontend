...
const ApplicationCard = ({ app, onDelete }) => {
    ...
    return (
        <div className="bg-white rounded-lg shadow-md overflow-hidden flex items-center p-5 max-w-lg">
            <img
                src={`http://localhost:5500/listings/${app.petImage}`}
                alt={app.petName}
                className="w-24 h-24 rounded-full object-cover mr-4"
            ...
            <div className="flex-grow">
                <div className="flex justify-between items-center mb-2">
                    <h3 className="text-xl font-semibold text-black">{app.petName}</h3>
                    <span className="px-2 py-1 rounded-full text-xs font-medium bg-[#E0D082] text-black">
                        Pending
                    </span>
                </div>
                <div className="flex space-x-4">
                    ...
                    <button
                        onClick={handleEditClick}
                        className="flex-1 bg-[#E0D082] text-black py-2 rounded-md hover:bg-[#d4c074] transition duration-300"
                    >
                        Edit Application
                    </button>
                    {/* Cancel Button */}
