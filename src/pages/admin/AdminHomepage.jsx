...
const Dashboard = () => {
  ...
  return (
    <div className="flex h-screen" style={{ backgroundColor: '#e0d082' }}>
      {/* Sidebar */}
      <div className="w-64" style={{ backgroundColor: '#e0d082', color: 'black' }}>
        <div className="flex items-center justify-center h-20 shadow-md">
          <img src={officiallogo} alt="Logo" className="h-10 w-auto mr-3" />
          <span className="text-2xl font-semibold">Paws&Homes</span>
        </div>
        <nav className="mt-5">
          {[
            ...
            <Link
              key={index}
              to={item.path}
              className="flex items-center mt-4 py-2 px-6 hover:bg-opacity-50 transition-colors duration-200"
              style={{ color: 'black' }}
            >
        ...
        <div className="container mx-auto px-6 py-8">
          ...
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <div
                key={index}
                className={`${stat.color} bg-opacity-40 shadow-lg rounded-md flex items-center justify-between p-3 border-b-4 border-${stat.color.split('-')[1]}-600 text-white font-medium group`}
              >
                <div className="flex justify-center items-center w-14 h-14 bg-white rounded-full transition-all duration-300 transform group-hover:rotate-12">
                  <svg
                    ...
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    className={`stroke-current text-${stat.color.split('-')[1]}-800 transform transition-transform duration-500 ease-in-out`}
                  >
                    <path
                      strokeLinecap="round"
