...
import { useParams } from 'react-router-dom';
import { toast } from "react-toastify";
import { getoneListing, getSingleApplicationApi, updateApplicationApi } from '../../apis/Api';
import officiallogo from '../../assets/logo1-removebg-preview.png';
...
const ApplicationDetails = () => {
  ...
  const handleApproveApplication = async (status) => {
    try {
      ...
      console.log('Sending update:', updatedApplication);
      const response = await updateApplicationApi(id, updatedApplication);
  ...
  if (!application) {
    return <div className="flex justify-center items-center h-screen text-2xl font-semibold text-black">Loading...</div>;
  }

  return (
    <div className="min-h-screen bg-[#E0D082] font-sans">
      {/* Navbar */}
      <nav className="bg-[#E0D082] shadow-md">
        <div className="container mx-auto px-6 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <img src={officiallogo} alt="Logo" className="h-8 w-auto mr-3" />
      ...
      {/* Content */}
      <div className="container mx-auto px-4 py-8">
        <div className="bg-white rounded-lg shadow-lg overflow-hidden border border-[#E0D082]">
          <div className="md:flex">
            {/* Left Column: Pet Details */}
            <div className="md:w-1/3 p-8 bg-[#E0D082]">
              <h2 className="text-2xl font-bold mb-6 text-black">Pet Details</h2>
              <div className="flex justify-center mb-6">
                <img
                  src={`http://localhost:5500/listings/${applicationsWithPetDetails.petImage}`}
                  alt="Pet"
            ...
            <div className="md:w-2/3 p-8">
              <h2 className="text-3xl font-bold mb-6 text-black">Applicant Details</h2>
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <p className="text-sm uppercase tracking-wide text-black">Name</p>
                  <p className="font-semibold text-lg text-black">{application.name}</p>
                </div>
                <div>
                  <p className="text-sm uppercase tracking-wide text-black">Age</p>
                  <p className="font-semibold text-lg text-black">{application.age}</p>
                </div>
                <div>
                  <p className="text-sm uppercase tracking-wide text-black">Occupation</p>
                  <p className="font-semibold text-lg text-black">{application.occupation}</p>
                </div>
                <div>
                  <p className="text-sm uppercase tracking-wide text-black">Email</p>
                  <p className="font-semibold text-lg text-black">{application.email}</p>
                </div>
                <div>
                  <p className="text-sm uppercase tracking-wide text-black">Phone</p>
                  <p className="font-semibold text-lg text-black">{application.phonenumber}</p>
                </div>
                <div>
                  <p className="text-sm uppercase tracking-wide text-black">Address</p>
                  <p className="font-semibold text-lg text-black">{application.address}</p>
                </div>
              </div>

              {/* Other Information */}
              <div className="mt-8">
                <h3 className="text-xl font-bold mb-4 text-black">Other Information</h3>
                <div className="bg-white rounded-lg p-4 border border-[#E0D082]">
                  <p className="mb-2"><span className="font-semibold">Other pets present:</span> {application.haveDog}</p>
                  <p className="mb-2"><span className="font-semibold">Housing Situation:</span> {application.livingSituation}</p>
                  <p className="mb-2"><span className="font-semibold">Reason for adoption:</span></p>
                  <p className="italic">{application.reasonsForAdopting}</p>
                </div>
              </div>
              ...
              <div className="mt-8">
                <button
                  onClick={() => setIsOpen(true)}
                  className="px-6 py-3 bg-[#E0D082] text-black font-semibold rounded-lg shadow-md hover:bg-[#d4c074] transition duration-300 ease-in-out"
                >
                  Review Application
                </button>
              </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-10 overflow-y-auto bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
            <div className="flex justify-between items-center border-b border-[#E0D082] p-4">
              <h3 className="text-lg font-medium text-black">Review Applicant</h3>
              <button
                onClick={() => setIsOpen(false)}
                className="text-black hover:text-gray-600 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-black rounded-full p-1"
              >
                <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
            ...
            <form className="p-4">
              <div className="mb-4">
                ...
                <textarea
                  onChange={(e) => setAdminMessage(e.target.value)}
                  id="message"
                  className="w-full h-32 px-3 py-2 text-black border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-black"
                  placeholder="Enter your message here..."
                ></textarea>
              </div>
              <div className="flex justify-end space-x-3">
                <button
                  type="button"
                  onClick={() => handleApproveApplication("rejected")}
                  className="px-4 py-2 text-sm font-medium text-black bg-[#E0D082] rounded-md hover:bg-[#d4c074] focus:outline-none"
                >
                  Reject
                </button>
                <button
                  onClick={() => handleApproveApplication("accepted")}
                  type="submit"
                  className="px-4 py-2 text-sm font-medium text-white bg-black rounded-md hover:bg-gray-700 focus:outline-none"
                >
                  Approve
                </button>
              </div>
